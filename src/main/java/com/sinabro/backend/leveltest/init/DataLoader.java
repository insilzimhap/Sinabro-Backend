...
public class DataLoader implements CommandLineRunner {
    ...
    // FK 의존 순서상 먼저 비울 것들
    private final LevelTestChoiceRepository levelTestChoiceRepository;
    private final ParentChoiceRepository parentChoiceRepository;
    private final LevelTestOptionRepository levelTestOptionRepository;
    ...
    public void run(String... args) {
        ...
        if (seedReset) {
            // 1) 선택/응답(choices) 삭제
            levelTestChoiceRepository.deleteAllInBatch();
            parentChoiceRepository.deleteAllInBatch();

            // 2) 옵션 삭제 (options)
            levelTestOptionRepository.deleteAllInBatch();
            parentOptionRepository.deleteAllInBatch();

            // 3) 질문 삭제 (questions)
            questionRepository.deleteAllInBatch();
            parentQuestionRepository.deleteAllInBatch();
        }
